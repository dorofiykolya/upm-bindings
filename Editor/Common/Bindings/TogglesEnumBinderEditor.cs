...
{
    [CustomEditor(typeof(TogglesEnumBinder), true), CanEditMultipleObjects]
    public class TogglesEnumBinderEditor : ABinderEditor
    {
        private void DrawEnumBindingProperties()
        {
            ...
            for (var i = 0; i < stateObjects.arraySize; i++)
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    var go = stateObjects.GetArrayElementAtIndex(i).FindPropertyRelative("TargetToggle");
                    ...
                    try
                    {
                        ...
                        go.objectReferenceValue =
                            EditorGUILayout.ObjectField(go.objectReferenceValue, typeof(Toggle), true);
                    }
                    catch
                    {
                        if (String.IsNullOrEmpty(enumPropVal.stringValue))
